...
function getTime(date) {
  ...
  const mitutes = String(new Date(date).getMinutes()).padStart(2, '0');
  return `${hours}:${mitutes}:${seconds}`;
}
...
function getNextFriday(date) {
  ...
  if (day === 5) {
    ...
    data.setUTCDate(data.getUTCDate() + ((5 - day + 7) % 7));
  }
  return data;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (new Date(dateEnd) - new Date(dateStart)) / (24 * 3600 * 1000) + 1;
}
...
function formatDate(date) {
  const data = new Date(date);
  const firstPartDate = `${data.getUTCMonth() + 1}/${data.getUTCDate()}/${data.getFullYear()}`;
  let secondPartDate = '';
  const seconds = String(data.getSeconds()).padStart(2, '0');
  const mitutes = String(data.getMinutes()).padStart(2, '0');

  if (data.getUTCHours() === 12) {
    secondPartDate = `${data.getUTCHours()}:${mitutes}:${seconds} PM`;
  } else if (data.getUTCHours() > 11) {
    secondPartDate = `${data.getUTCHours() - 12}:${mitutes}:${seconds} PM`;
  } else {
    secondPartDate = `${data.getUTCHours()}:${mitutes}:${seconds} AM`;
  }
  return `${firstPartDate}, ${secondPartDate}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstThurday = new Date(
    year,
    0,
    1 + ((11 - new Date(year, 0, 1).getDay()) % 7)
  );
  const firstMonday = new Date(firstThurday).setDate(
    firstThurday.getDate() - 3
  );
  const diffIndays = (currentDate - firstMonday) / (24 * 3600 * 1000);
...
function getNextFridayThe13th(date) {
  ...
  const firstFriday = new Date(
    year,
    0,
    1 + ((12 - new Date(year, 0, 1).getDay()) % 7)
  );
  const nextFriday = firstFriday;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      result.push(formatDateResult(currentDate));
